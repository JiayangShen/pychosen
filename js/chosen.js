	...
	$.fn.extend({
		chosen: function(options) {
			...
			return this.each(function(input_field) {
                ...
                var data = cfg.data, pyData = [];
                var valField = cfg.valField, txtField = cfg.txtField, pyField = cfg.pyField;
                
                if(data && data.length && valField && txtField)
                {
                    var selectHtml = isEmpty && cfg.showDefault ? '<option></option>' : '';
                    
                    for(var i = 0, item; item = data[i]; i++)
                    {
                        if(!item.isOptgroup && item[pyField]) pyData.push(item);
                        if(item.isOptgroup)
                        {
                            if(isEmpty) {
                                selectHtml +='<optgroup label="' + (item.optgroupLabel || '') + '" optgroup-id="' + (item.optgroupID || '') + '">';
                            }
                            var optItems = item.optItems;
                            for(var j = 0, optItem; optItem = optItems[j]; j++)
                            {
                                if(optItem[pyField]) pyData.push(optItem);
                                if(isEmpty) {
                                    selectHtml += '<option value="' + optItem[valField] + '">' + optItem[txtField] + '<option>';
                                }
                            }
                            if(isEmpty) selectHtml += '</optgroup>';
                        }
                        else
                        {
                            if(isEmpty) {
                                selectHtml += '<option value="' + item[valField] + '">' + item[txtField] + '<option>';
                            }
                        }
                    }
                    
                    if(isEmpty) $this.html(selectHtml);
                }
                
                if(pyData && pyData.length && valField && txtField)
                {
                    var matcher = new Pymatch([]);
                    var matchData = [];
                    
                    for(var k = 0, pyItem; pyItem = pyData[k]; k++)
                    {
                        matchData.push($.extend({}, pyItem, {name: pyItem[txtField], py: pyItem[pyField]}));
                    }
                    
                    matcher.setNames(matchData);
                    ...
                    var getMatchedFn = function (searchText)
                    {
                        ...
                        for(var m = 0, mItem; mItem = matchedSections[m]; m++)
                        {
                            matchedOptions[mItem[0][valField]] = true;
                        }
                        
                        return matchedOptions;
                    };
                    
                    cfg.getMatchedFn = getMatchedFn;
                }
                
                $this.data('chosen', new Chosen(this, cfg));
			});
		}
	});
